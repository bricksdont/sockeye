...
class ReconstructionScoringModel(model.SockeyeModel):
    """
    ReconstructionScoringModel is a ReconstructionModel that scores a pair of sentences.
    It takes a list of (identical) source sentences and their nbest hypotheses (produced by translating with a model trained as training.ReconstructionModel), and it returns the combined score of the translation and the reconstruction (weigthed by reconstruction_lambda). 
    ...
    def _initialize(self,
        ...
        def sym_gen(seq_lens):
            ...
            reconstructed_logits = mx.sym.reshape(data=reconstructed_logits, shape=(-4, -1, source_embed_seq_len, 0))

            if self.softmax_temperature is not None:
                logits = logits / self.softmax_temperature
                reconstructed_logits = reconstructed_logits / self.softmax_temperature
            ...
            source_dists = mx.sym.softmax(data=reconstructed_logits, axis=2, name=C.SOFTMAX_RECONSTRUCTION_OUTPUT_NAME)

            # Select the label probability, then take their logs.
            # probs and scores: (batch_size, target_seq_len)
            target_probs = mx.sym.pick(target_dists, labels)
            target_scores = mx.sym.log(target_probs)
            
            source_probs = mx.sym.pick(source_dists, source_labels)
            source_scores = mx.sym.log(source_probs)

            if self.score_type == C.SCORING_TYPE_NEGLOGPROB:
                target_scores = -1 * target_scores
                source_scores = -1 * source_scores

            # zeros and sums: (batch_size,)
            target_zeros = mx.sym.zeros_like(target_scores)
            target_sums = mx.sym.sum(mx.sym.where(labels != 0, target_scores, target_zeros), axis=1) 
            
            source_zeros = mx.sym.zeros_like(source_scores)
            source_sums = mx.sym.sum(mx.sym.where(source_labels != 0, source_scores, source_zeros), axis=1) 

            # Return the sums and the target distributions
            # sums: (batch_size,) target_dists: (batch_size, target_seq_len, target_vocab_size)
            return mx.sym.Group([target_sums, source_sums, target_dists, source_dists]), data_names, label_names
...
class Scorer:
    """
    Scorer class takes a ReconstructionModel and uses it to score a stream of parallel sentences.
