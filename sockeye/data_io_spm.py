...
def get_training_data_iters(sources: List[str],
    ...
    if use_spm:
        sentencepiece_sampler = sentencepieceSampler(
                                spm_alpha=spm_alpha,
...
class sentencepieceSampler:
    ...
    def resample(self, data):
           ...
           length_statistics = analyze_sequence_lengths(new_sources, new_target, 
                                                         self.source_vocabs, self.target_vocab, self.max_seq_len_source, self.max_seq_len_target)
           buckets = define_parallel_buckets(self.max_seq_len_source, 
           ...
           sources_sentences, target_sentences = create_sequence_readers(new_sources, new_target, self.source_vocabs, self.target_vocab)
           data_statistics = get_data_statistics(sources_sentences, target_sentences, buckets,
                                          length_statistics.length_ratio_mean, length_statistics.length_ratio_std,
...
class ParallelSampleIter(BaseParallelSampleIter):
    ...
    def __init__(self,
                 ...
                 sentencepiece_sampler: Optional[sentencepieceSampler] = None) -> None:
        super().__init__(buckets=buckets, batch_size=batch_size, bucket_batch_sizes=bucket_batch_sizes,
                         source_data_name=source_data_name, target_data_name=target_data_name,
                         label_name=label_name, num_factors=num_factors, permute=permute, dtype=dtype)
    ...
    def reset(self):
        ...
        self.curr_batch_index = 0
        if self.use_spm:
            logger.logging("resetting with spm")
            self = self.spm_sampler.resample(self.data)
